...
from steerers.DeDoDe.model_zoo.dedode_models import dedode_detector_L, dedode_descriptor_B
from steerers.rotation_steerers.steerers import DiscreteSteerer, ContinuousSteerer
from steerers.rotation_steerers.matchers.max_similarity import MaxSimilarityMatcher, ContinuousMaxSimilarityMatcher
...
def keypoint_matching(img_path1, img_path2):
    ...
    detections_A = detector.detect_from_path(img_path1, num_keypoints = 50)
    keypoints_A, P_A = detections_A["keypoints"], detections_A["confidence"]
    detections_B = detector.detect_from_path(img_path2, num_keypoints = 50)
    ...
    descriptions_A = descriptor.describe_keypoints_from_path(img_path1, keypoints_A)["descriptions"]
    descriptions_B = descriptor.describe_keypoints_from_path(img_path2, keypoints_B)["descriptions"]
    ...
    point_list1 = matches_A.cpu().tolist()
    point_list2 = matches_B.cpu().tolist()
    result1 = [tuple(point) for point in point_list1]
    result2 = [tuple(point) for point in point_list2]
    return result1, result2

def pixel_to_latlon(tif_path, x, y):
    """
    将tif图片上的像素坐标(x, y)转换为实际的经纬度(lat, lon)。

    参数:
        tif_path (str): 输入tif文件的路径。
        x (int): 像素坐标的x位置 (列索引, 0 < x < tif宽度)。
        y (int): 像素坐标的y位置 (行索引, 0 < y < tif高度)。

    返回:
        (lat, lon): 该像素对应的实际地理坐标 (纬度, 经度)。
    """
    # 打开tif文件
    with rasterio.open(tif_path) as dataset:
        # 将行列像素索引转换为地理坐标
        lon, lat = dataset.transform * (x, y)
        return lat, lon

def haversine_distance(lat1, lon1, lat2, lon2):
    """
    计算两个地球表面坐标之间的地理距离（单位：米）
...
def compute_localization(query_im, query_im_path, query_res, index_info_csv, db_dir, matcher, extend_size=2000):
    ...
    for ind in query_res:
        ...
        # 定义保存路径
        save_dir = "/home/lwy/code_1/crop_images"
        # 定义保存文件名
        crop_image_name = f"crop_{ind}.tif"
        crop_image_path = os.path.join(save_dir, crop_image_name)

        #保存切割后的图像为tif格式
        cv2.imwrite(crop_image_path, match_im_crop)
        print(f"Saved cropped image to {crop_image_path}") 

        test_pts, match_pts = [], []
         
        test_pts, match_pts = keypoint_matching(query_im_path, crop_image_path)     
...
if __name__ == '__main__':
    ...
    with open(args.output_file, 'w') as result_writer: 
        ...
        for i in tqdm(range(len(test_fnames))): 
            ...
            query_im2 = Image.open(test_fnames[i])
            # print("test_fnames 内容为：")
            # print("\n")
            # print(test_fnames[i])
            query_im_path = test_fnames[i]
            df = pd.read_csv("05.csv")
            csv_file_name = query_im_path.split('/')[-1]
            print(csv_file_name)
            print("\n")
            #找到对应图片的phi值，进行旋转
            matched_row = df[df['filename'] == csv_file_name]
            if not matched_row.empty:
                phi1_value = matched_row['Phi1'].iloc[0]  # 提取 Phi1 值
                print(f"Phi1 value: {phi1_value}\n")
            
            theta = -phi1_value
            rotated_image = query_im2.rotate(theta)
            # 获取旋转后图像的边界框
            box = rotated_image.getbbox()
            # 根据边界框裁剪图像,得到用于匹配的预处理查询图像
            final_query_im = rotated_image.crop(box)
            # 确保输出目录存在
            output_directory = "/home/lwy/code_1/demo_query_images2/rotated"
            final_query_im_name = csv_file_name.replace(".jpg","") +"_"+"rotated"+"_"+".jpg"
            output_path = os.path.join(output_directory,final_query_im_name)
            final_query_im.save(output_path)
            # print("成功保存图片："+final_query_im_name)

            find_flag =False
            # find_flag, result, match_im_name = compute_localization(query_im, query_im_path, indices[i], index_info_csv, args.db_dir, matcher, args.extend_size)
            find_flag, result, match_im_name = compute_localization(query_im2, output_path, indices[i], index_info_csv, args.db_dir, matcher, args.extend_size)
