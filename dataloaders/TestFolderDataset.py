...
def resize_and_pad(image, min_size=322, max_size=644):
    # 获取图像的原始尺寸
...
class TestFolderDataset(Dataset):
    def __init__(self, im_dir= '../data/test_images/',  input_transform = None, ext=['.jpg'], max_size=644):
        
        self.input_transform = input_transform
        self.image_dir = im_dir 
    
        self.images = []
        for _ext in ext:
           self.images += glob.glob(os.path.join(self.image_dir,'*'+_ext))   
        self.images = sorted(self.images)
        # print('self.images[0:10]', self.images[0:10])
...
class TestImageCropDataset(Dataset):
    def __init__(self, im_path= '../data/test_images/', crop_size=[560], step_size=None, input_transform = None):
